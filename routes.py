...
def click():
    """
    DEPRECATED: This function was used to test the front and back end
       integrations

    Returns:
        json: A 'success' string to be printed on the front end.
    """
...
def turn_off_motors():
    """
    This endpoint unlocks/turns off the motors so that drag and teach mode
       can be used. Allows moving the robot manually by hand.
...
def turn_on_motors():  # NOTE THIS FUNCTION DOES NOT YET WORK
    """
    This endpoint is supposed to lock the motors and end the ability to
       move the robot manually by hand. Currently this is not working as
       expected.
...
def send_pose():
    """
    This endpoint allows the front end to send a single string of angles
       also known as a pose. The robot will be given the command to immediately
       jump to that position.

    Returns:
        json: A string of the list of angles that the robot moved to.
    """
    if request.method == "POST":
        print("received api request")
...
def get_pose():
    """
    Pings the robot to get a list of angles of its current position and sends
       that data in a response.

    Returns:
       json: A list of the robot's current angles
    """
...
def send_angles_sequence():
    """
    Instructs the robot to move through a sequence of 20 angles while waiting
       1 second in between each movement.

    Returns:
          json: A string of the full 20 step sequence of angles that the robot 
          was instructed to move through.
    """
    if request.method == "POST":
        print("received api request")
        ...
        sp = 80
        mc = MyCobot("/dev/ttyAMA0", 1000000)
        for angle_1, angle_2, angle_3, angle_4, angle_5, angle_6 in zip(
            angles_1, angles_2, angles_3, angles_4, angles_5, angles_6
        ):
...
def get_all_animation_files():
    """
    Checks the directory where animation files are stored and returns a list
    of existing filenames in a response.

    Returns:
        json: A list of filenames found in the directory where animation files
          are saved.
    """
    if request.method == "GET":
